...
import com.app.pdfcurldemo.curl.CurlPage;
import com.app.pdfcurldemo.curl.CurlView;
...
public class PDFCurlViewAct extends Activity {
    ...
    public class PDFCurlThread extends Thread {
        ...
        private Bitmap render_page(int pageno) {
            int w = m_w;
            if (m_w > m_h) w = m_w / 2;
            Bitmap bmp = Bitmap.createBitmap(w, m_h, Bitmap.Config.ARGB_8888);
            Page page = m_doc.GetPage(pageno);
            float scale1 = w / m_doc.GetPageWidth(pageno);
            float scale2 = m_h / m_doc.GetPageHeight(pageno);
            if (scale1 > scale2) scale1 = scale2;
            float x0 = (w - scale1 * m_doc.GetPageWidth(pageno)) / 2;
